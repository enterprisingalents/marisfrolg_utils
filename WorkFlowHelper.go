...
func CreateWorkFlow( businessObj map[string]interface{},userId , UserName , workFlowTag ,token  string) (data string, err error) {
	var(
	...
	//请求地址
	//juheURL := "http://192.168.11.234:7999/v1/WorkFlow" + "/Create"
	createUrl := WF_CONN + "/Create"
	//初始化参数
	param := url.Values{}
	...
	if responseBytes, err = HttpPost2(createUrl, param, "", token);err!=nil{
...
//执行审核流程，通过、拒绝、驳回等
func ActionWorkFlow(workFlowID string, node *Node, token string) (data string, err error) {
	var(
		responseBytes,nodeBytes []byte
	)
	actionUrl := WF_CONN + "/Action" //初始化参数
	param := url.Values{}                //初始化参数
	param.Set("workFlowId", workFlowID)
	nodeBytes, _ = json.Marshal(node)
	responseBytes, err = HttpPost2(actionUrl, param, string(nodeBytes), token)
	if err != nil {
